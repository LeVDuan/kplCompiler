...
void printUsage(void) {
  printf("Usage: kplexe input [-s=stack_size] [-c=code_size] [-debug] [-dump]\n");
...
int main(int argc, char *argv[]) {
  ...
  if (compile(argv[1]) == IO_ERROR) {
    printf("kplexe: Can\'t read input file!\n");
    return -1;
  }
  serialize("./kplCompiler/output");
  cleanCodeBuffer();

  f = fopen("./kplCompiler/output","r");

  initVM();
  if (loadExecutable(f) == 0) {
    printf("kplexe: Wrong executable format!\n");
    fclose(f);
    cleanVM();
    return -1;
  }
